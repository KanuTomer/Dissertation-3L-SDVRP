...
from comparison_models.proposed_model.config import MODEL_NAME
...
def run_from_config(config: dict):
    ...
    model_name = str(config.get("model_name_override") or MODEL_NAME)
    outputs_root = Path(config["outputs_root"]) if config.get("outputs_root") else None
    config["progress_label"] = config.get("progress_label", f"{model_name} {dataset_name} seed {seed}")

    out_dir = get_output_dir(model_name, dataset_path, outputs_root=outputs_root)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run proposed_model experiments across generated datasets.")
    ...
    parser.add_argument("--disable-adaptive-decoding", action="store_true")
    parser.add_argument("--disable-tiny-route-repair", action="store_true")
    parser.add_argument("--disable-customer-relocation-repair", action="store_true")
    parser.add_argument("--disable-route-balance-mutation", action="store_true")
    parser.add_argument("--disable-final-best-refinement", action="store_true")
    ...
    for dataset_path in dataset_paths:
        ...
        for seed in range(args.start_seed, args.end_seed + 1):
            print(f"Running proposed_model seed {seed}", flush=True)
            cfg = {
                "dataset_path": str(dataset_path),
                "seed": seed,
                ...
                "enable_adaptive_decoding": not args.disable_adaptive_decoding,
                "enable_tiny_route_repair": not args.disable_tiny_route_repair,
                "enable_customer_relocation_repair": not args.disable_customer_relocation_repair,
                "enable_route_balance_mutation": not args.disable_route_balance_mutation,
                "enable_final_best_refinement": not args.disable_final_best_refinement,
                "verbose": args.verbose,
            }
            result = run_from_config(cfg)
            print(
                f"Completed proposed_model seed {seed} | "
