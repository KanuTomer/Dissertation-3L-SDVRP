# ga_runner.py — patched: accept dataset path OR preloaded dict, add verbose logging
import time
import random
from math import sqrt
from pathlib import Path

from .crossover import order_crossover
from .mutation import swap_mutation
from .selection import tournament_select

from dataset_generation.loaders.route_evaluator import load_merged, evaluate_route

class GARunner:
    def __init__(self, config: dict):
        self.config = config or {}
        self.pop_size = int(self.config.get("pop_size", self.config.get("population_size", 80)))
        self.gens = int(self.config.get("gens", self.config.get("num_generations", 200)))
        self.cx_prob = float(self.config.get("cx_prob", self.config.get("crossover_prob", 0.9)))
        self.mut_prob = float(self.config.get("mut_prob", self.config.get("mutation_prob", 0.2)))
        self.max_boxes_per_route = int(self.config.get("max_boxes_per_route", self.config.get("maxboxes", 48)))
        self.penalty_factor = float(self.config.get("penalty_factor", self.config.get("alpha", 1e8)))
        self.seed = int(self.config.get("seed", 42))
        # verbosity
        self.verbose = bool(self.config.get("verbose", False))

    @staticmethod
    def euclid(a, b):
        return sqrt((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2)

    @staticmethod
    def route_distance(depot, customers_map, route):
        if not route:
            return 0.0
        dist = 0.0
        first = customers_map[route[0]]
        dist += GARunner.euclid((depot[0], depot[1]), (first["x"], first["y"]))
        for i in range(len(route) - 1):
            c1 = customers_map[route[i]]
            c2 = customers_map[route[i + 1]]
            dist += GARunner.euclid((c1["x"], c1["y"]), (c2["x"], c2["y"]))
        last = customers_map[route[-1]]
        dist += GARunner.euclid((last["x"], last["y"]), (depot[0], depot[1]))
        return dist

    @staticmethod
    def build_customer_boxcount_map(customers):
        return {c["customer_id"]: len(c.get("assigned_boxes", [])) for c in customers}

    @staticmethod
    def decode_by_boxcount(order, cust_boxcount_map, max_boxes_per_route=48):
        routes = []
        cur = []
        cur_boxes = 0
        for cid in order:
            b = cust_boxcount_map.get(cid, 0)
            if cur and (cur_boxes + b > max_boxes_per_route):
                routes.append(cur)
                cur = [cid]
                cur_boxes = b
            else:
                cur.append(cid)
                cur_boxes += b
        if cur:
            routes.append(cur)
        return routes

    def evaluate_permutation(self, merged_source, perm):
        # merged_source may be a path or a preloaded dict; handle both
        if isinstance(merged_source, (dict, )):
            # if dict, we expect the structure: {'inst_name', 'container', 'customers', 'boxes'}
            d = merged_source
            inst_name = d.get("inst_name", "inst")
            container = d.get("container", {})
            customers = d.get("customers", [])
            boxes = d.get("boxes", [])
            # For evaluate_route we still pass the dict file path signature by writing a temp file could be heavy.
            # Instead, if evaluate_route expects path we will provide a minimal stub branch above that handles dicts.
            # Here we will call evaluate_route with the merged_source itself if it can handle dicts.
            eval_input = merged_source
        else:
            eval_input = merged_source

        if isinstance(eval_input, dict):
            # build structures directly
            cust_box_map = GARunner.build_customer_boxcount_map(eval_input.get("customers", []))
            routes = GARunner.decode_by_boxcount(perm, cust_box_map, max_boxes_per_route=self.max_boxes_per_route)
            if eval_input.get("customers"):
                depot = (eval_input["customers"][0]["x"], eval_input["customers"][0]["y"])
            else:
                depot = (0,0)
            customers_map = {c["customer_id"]: c for c in eval_input.get("customers", [])}
        else:
            # eval_input is path string
            inst_name, container, customers, boxes = load_merged(eval_input)
            cust_box_map = GARunner.build_customer_boxcount_map(customers)
            routes = GARunner.decode_by_boxcount(perm, cust_boxcount_map := cust_box_map, max_boxes_per_route=self.max_boxes_per_route)
            depot = (customers[0]["x"], customers[0]["y"]) if customers else (0,0)
            customers_map = {c["customer_id"]: c for c in customers}

        total_distance = 0.0
        infeasible_count = 0
        details = []
        for r in routes:
            dist = GARunner.route_distance(depot, customers_map, r)
            # call evaluate_route; allow it to accept either path or dict (our route_evaluator handles both)
            pack = evaluate_route(eval_input, r)
            feasible = pack.get("feasible", False)
            if not feasible:
                infeasible_count += 1
            total_distance += dist
            details.append({
                "route": r,
                "distance": dist,
                "feasible": feasible,
                "boxes_total": pack.get("boxes_total"),
                "boxes_packed": pack.get("boxes_packed"),
                "fill_rate": pack.get("fill_rate"),
            })
        if infeasible_count == 0:
            score = total_distance
        else:
            score = self.penalty_factor * infeasible_count + 1e-3 * total_distance
        return score, {"total_distance": total_distance, "infeasible_count": infeasible_count, "routes": details}

    def run(self, merged, pop_size=None, gens=None, cx_prob=None, mut_prob=None, seed=None, verbose=None):
        pop_size = int(pop_size if pop_size is not None else self.pop_size)
        gens = int(gens if gens is not None else self.gens)
        cx_prob = float(cx_prob if cx_prob is not None else self.cx_prob)
        mut_prob = float(mut_prob if mut_prob is not None else self.mut_prob)
        seed = int(seed if seed is not None else self.seed)
        if verbose is not None:
            self.verbose = bool(verbose)

        random.seed(seed)

        # if merged is a dict, use directly; if path, GARunner will call load_merged when needed
        if isinstance(merged, (dict, )):
            cust_ids = [c["customer_id"] for c in merged.get("customers", [])]
        else:
            inst_name, container, customers, boxes = load_merged(merged)
            cust_ids = [c["customer_id"] for c in customers]

        n = len(cust_ids)
        pop = [random.sample(cust_ids, n) for _ in range(pop_size - 2)]
        pop.append(cust_ids[:])
        pop.append(list(reversed(cust_ids)))

        fitness_cache = {}

        def fitness(ind):
            key = tuple(ind)
            if key in fitness_cache:
                return fitness_cache[key]
            val = self.evaluate_permutation(merged, ind)
            fitness_cache[key] = val
            return val

        best = None
        best_s = float("inf")
        history = []
        start = time.time()

        for g in range(gens):
            scored = [fitness(ind)[0] for ind in pop]
            for i, s in enumerate(scored):
                if s < best_s:
                    best_s = s
                    best = pop[i][:]
            ranked = sorted(zip(scored, pop), key=lambda x: x[0])
            elite_n = max(4, int(0.05 * pop_size))
            new_pop = [ranked[i][1][:] for i in range(elite_n)]
            while len(new_pop) < pop_size:
                p1 = tournament_select(pop, scored)
                p2 = tournament_select(pop, scored)
                if random.random() < cx_prob:
                    child = order_crossover(p1, p2)
                else:
                    child = p1[:]
                child = swap_mutation(child, mut_prob)
                new_pop.append(child)
            pop = new_pop
            history.append(best_s)

            # verbose progress print
            if self.verbose:
                if gens <= 10 or g % max(1, (gens // 10)) == 0 or g == gens - 1:
                    print(f"[GARunner] gen {g+1}/{gens} best_score={best_s:.6f}")

        duration = time.time() - start
        best_val, best_info = fitness(best)
        return {
            "best_order": best,
            "best_score": best_val,
            "best_info": best_info,
            "history": history,
            "duration": duration,
        }
