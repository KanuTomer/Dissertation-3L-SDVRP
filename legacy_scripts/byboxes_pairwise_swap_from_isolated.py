# byboxes_pairwise_swap_from_isolated.py
import json, time
...
IN_J = "route_ga_vlr_byboxes_out/route_ga_vlr_byboxes_isolated.json"   # start from isolated order
OUT_J = "route_ga_vlr_byboxes_out/route_ga_vlr_byboxes_swapped_from_isolated.json"

# Tunable params: keep conservative but allow more candidates
MAX_BOXES = 40
TOP_K_HEAVY = 6
MAX_SUCCESSFUL_SWAPS = 400
...
def load_order():
    ...
    # isolated file uses key 'best_order_isolated'
    return d.get('best_order_isolated', d.get('best_order_fixed', d.get('best_order')))
...
def main():
    ...
    while swaps < MAX_SUCCESSFUL_SWAPS and time.time()-start < TIME_LIMIT and improved:
        ...
        if not infeasible_idxs:
            break
        # try candidate pairs (infeasible <-> all)
        pairs = []
